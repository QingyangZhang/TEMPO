...
def grade_answer_math_verify(given_answer: str, ground_truth: str) -> tuple[bool, str, str]:
    if (len(given_answer) > 128 and repeatness(given_answer)) or \
        (len(ground_truth) > 128 and repeatness(ground_truth)):
        return False, given_answer, ground_truth
    ...
    given_answer_boxed = "\\boxed{" + given_answer + "}"
    
    #print(f"{ground_truth_boxed}, {given_answer_boxed}")

    score, _ = verify_func([ground_truth_boxed], [given_answer_boxed])
    
    return float(score) >= 1.0, given_answer, ground_truth
...
def retry(max_attempts:int=3, delay:int=1, print_trace_back=False, return_error_info=False):
    assert isinstance(max_attempts, int) and isinstance(delay, int), 'max_attempts and delay must be integers'
...
class Model_args:
    ...
    base_url="http://10.102.247.26:34886/v1"  # Anonymized model path or URL
    max_tokens = 4096
    temperature = 0.1
...
def grade(model_answer: str, gt_answer: str, is_matched: bool, problem=None, use_xverify=False):
    ...
    if not correct:
        try:
            ...
            print(f"[WARNING] math-verify failed when grading {model_answer}, {gt_answer} due to {e}")
...
def get_answer_str(s: str, return_origin=False, num_answers=1):
    ...
    answer_list = [remove_boxed(b) if remove_boxed(b) else "" for b in boxed_list]
...
def answer_tag_reward_fn_for_r1(model_output: str, ground_truths, problem=None, points=None, use_xverify=False):
    ...
    for extracted_pred, ground_truth in zip(extracted_answers, ground_truths):
        try:
            ...
            score = 0.0
            score_by = "grading_failed"
            extracted_pred = extracted_pred
            extracted_gt = ground_truth
            print(f"[WARNING] Fail to grade {extracted_pred}, {ground_truth} due to Error: {e}")
...
def compute_score_p1(model_output, label, points=None, question="", use_xverify=True):
    # todo: add use_xverify as a config
    ground_truths = label
    if isinstance(ground_truths, str):
        ground_truths = [ground_truths]
    # print(f"[DEBUG] points: {points}, ground_truths: {ground_truths}")
    score, point, extracted_pred, extracted_gt, scored_by, score_noxverify, point_noxverify = answer_tag_reward_fn_for_r1(model_output, ground_truths, question, points, use_xverify=True)
...
if __name__ == "__main__":
    # Example usage
    model_output = r"### Final Answer:$$\boxed{3/5}$$"
    ground_truth = [r'\boxed{\sqrt{0.36}}']
